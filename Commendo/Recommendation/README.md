...
| `trendOrRelatedSignal` | 상세 응답에 관련 도서가 있으면 약한 관련 신호로 `0.2` 부여 |
...
북마크 하나의 선호 가중치는 평점과 최근성을 곱해 계산합니다.
...
| `recencyScore` | 가장 최근 북마크가 180일 이내일수록 높은 값 |
| `publicationAge` | 출간 10년 이내일수록 높은 값 |
| `trendOrRelatedSignal` | 관련 도서 신호가 있으면 부여되는 약한 보정값 |
...
- `recencyScore`는 후보와 실제로 매칭된 선호 신호의 나이가 아니라 가장 최근 북마크 날짜만 봅니다.
- `trendOrRelatedSignal`은 현재 관련 도서 존재 여부에 가까우며, 트렌드/신간/검색/관련 도서 출처별 신호로 분리되어 있지 않습니다.
