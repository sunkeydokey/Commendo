...
const RESPONSE_MAX_AGE_SECONDS = 86400;
...
export default {
  ...
  async scheduled(controller: ScheduledController, env: Env, ctx: ExecutionContext): Promise<void> {
    void controller;
    ctx.waitUntil(refreshNewArrivalSnapshots(env));
  }
};
...
async function handleNewArrivals(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  const url = new URL(request.url);
  ...
  const cacheKey = new Request(buildNewArrivalCacheKey(url, params.type, params.page, params.pageSize), {
    method: "GET"
  });
  ...
  const response = json({
    ...
    items: rows.results.map(newArrivalBookFromRow)
  });
...
async function fetchAladinNewArrivals(env: Env, type: NewArrivalType): Promise<NewArrivalBook[]> {
  ...
  for (let page = 1; page <= totalPages && books.length < NEW_ARRIVAL_MAX_ITEMS; page += 1) {
    ...
    books.push(...items.map(normalizeAladinBook));
...
async function saveNewArrivalSnapshot(
  db: D1Database,
  type: NewArrivalType,
  snapshotDate: string,
...
async function getLatestNewArrivalSnapshot(
  db: D1Database,
  type: NewArrivalType
): Promise<NewArrivalSnapshotRow | null> {
  return db.prepare(
...
function buildNewArrivalCacheKey(
  url: URL,
  type: NewArrivalType,
  page: number,
  pageSize: number
...
function normalizeAladinBook(item: AladinBookItem): NewArrivalBook {
  return {
...
function newArrivalBookFromRow(row: NewArrivalBookRow): NewArrivalBook {
  return {
    title: row.title,
    author: row.author,
